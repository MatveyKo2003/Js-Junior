...
У сервера много сединений у локальньного компьютера одно
...
**Ответ:**Написать дату файла когда он был изменен или указать версию файла (1.1.2)
...
**Ответ:**Нет потому что ты сохранил измененный файл и старую версию файла никак не вернуть для этого и создали СКВ.
...
**Ответ:**Да ведь СКВ сохраняет предыдущие версии  файлов 
...
console.log('Hello Ivan')
```
**Ответ:**Я понял только как происходит процесс слияния веток а файлов нет
...
```
Когда я узнаю об этом изменении?
**Ответ:**Как только у Ивана появится интернет

Как мне внести измения в этот файл? Где я смогу внести изменения в этот файл?
**Ответ:**создать pull request и изменять файл,на сервере
...
```js git checkout <hash> ``` 
 
Чтобы добавить файл в индекс для коммита (перевести его из состояния unstaged в состояние staged), нужно выполнить команду 
```js git add filename.js ``` 
...
-Только что инициализированный репозиторий содержит только одну ветку -master; git создает ветку мастер
-  “- -”
-Команда git branch показывает не только все доступные ветки, но так же и отмечает текущую символом *.
-git merge;git rebase- сливают ветки между собой
